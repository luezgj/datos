	...
	case RANDOM_RESET: {
		...
		for (i = 0; i < (Ne / 2); i++) for (j = 0; j < M; j++) {
			if (ProbabilisticChoice(randomMutProb)) {
				do {
					...
					if (r == i) exists = 1;
					base = (i * M) + 1;									// no self-synapses
					for (k = 0; k < M; k++) if (arch[base++] == r) exists = 1;	// synapse already exists  
				} while (exists == 1);
				arch[index] = r;
			}
			index++;
		...
		for (i = Ne + Ni + numDistSensors; i < Ne + Ni + numDistSensors + ceil(float(numMotorNeurons) / 2); i++) for (j = 0; j < M; j++) {
			if (ProbabilisticChoice(randomMutProb)) {
				do {
					exists = 0;		// avoid multiple synapses
					r = UniformRandomInteger(0, Ne);						// sensor -> to any interneuron 
					if (r == i) exists = 1;			// no self-synapses
					base = (ceil((Ne + Ni + numDistSensors) / 2) * M) + ((i - (Ne + Ni + numDistSensors)) * M) + 1;
					for (k = 0; k < M; k++) if (arch[base++] == r) exists = 1;	// synapse already exists  
				} while (exists == 1);
...
bool TSearch::exists(int from, int to, TVector<int>& arch) {
	//cout << "Comprobando si hay conexion desde " << from << " hasta "<< to <<endl;
	int base = (((from - 1) / (M)) * M) + 1;
	//cout << "base: " << base << ". Recorro la fila: ";
	for (int j = 0; j < M; j++) {
		//cout << " " << base << ",";
		if ((base != from) && (arch[base] == to)) {
			//cout << "Devuelvo true";
			return true;
		}
		base++;
...
int TSearch::getNewConnection(int from) {
	if (from <= (Ne / 2)*M) { //excitatory neuron
		...
		if (from <= ((Ne + Ni) / 2)*M) { // inhibitory neuron
			...
			if (from <= (((Ne + Ni) / 2) * M) + (ceil(numDistSensors / 2) * inConnections)) { int r = UniformRandomInteger(0, Ne + Ni - 1); return r; }  //sensor neuron
...
void TSearch::UniformCrossover(genotype& g1, genotype& g2)
{
	...
	for (int i = 1; i <= archGenotypeSize; i++) {
		...
		while (exists(i, g1.ints[i], g1.ints)) {
			...
			//cout << "gen1 cambiado por" << g1.ints[i] << endl;
		}

		//cout << "gen2" << endl;
		while (exists(i, g2.ints[i], g2.ints)) {
			g2.ints[i] = getNewConnection(i);
			//cout << "gen2 cambiado por "<< g2.ints[i];
		}
	}
...
//At end set next population into parents vector.
void TSearch::SelectNextPopulation(TVector<genotype >& parents, TVector<double> & parentsPerf) {
//ofsspring are in Population, offspring performance are in Perf	

	TVector<genotype > TempPopulation;
	TempPopulation.SetBounds(1,parents.Size());

	TVector<double> TempPerf;
	TempPerf.SetBounds(1, parents.Size());

	int eliteCount = EFraction * parents.Size();

	//parents elite passes to next population
	for (int i = 1; i <= (int)floor(EFraction * parents.Size() + 0.5);i++) {
		TempPopulation[i] = parents[i];
		TempPerf[i] = parentsPerf[i];
	}

	//offspring elite passes to next population
	for (int i = 1; i <= (int)floor(EFraction * parents.Size() + 0.5); i++) {
		TempPopulation[i+ eliteCount] = Population[i];
		TempPerf[i+ eliteCount] = Perf[i];
	}
	
	TVector<bool> used;
	used.SetSize(parents.Size());
	used.FillContents(false);


	int individual, MaxindividualIndex;  //Mix= max-min
	double performance;
	genotype gen;
	double maxPerformance;
	genotype maxIndividual;
	int fromOffspringCount = floor((parents.Size() - (eliteCount*2)) * OffspringPercentageSurvivors);
	int lastOffspringSurvivorIndex = (1 + (eliteCount*2)) + fromOffspringCount;

	//fill with offspings
	for (int i = (eliteCount*2)+1; i <= lastOffspringSurvivorIndex; i++) {
		maxPerformance = 0.0;
		for (int competitor = 1; competitor <= TourSize; competitor++) {
			individual = rs.UniformRandomInteger(eliteCount + 1, Population.Size());
			while (used[individual]) {
				individual = rs.UniformRandomInteger(eliteCount + 1, Population.Size());
			}
			performance = Perf[individual];
			gen = Population[individual];
			//cout << "competidor of: " << performance << endl;

			if (performance > maxPerformance) {
				maxPerformance = performance;
				MaxindividualIndex = individual;
				maxIndividual = gen;
			}
		}
		TempPopulation[i] = maxIndividual;
		//cout << "Elegido of: " << maxPerformance<< endl;
		TempPerf[i] = maxPerformance;
		used[MaxindividualIndex] = true;
	}
	used.FillContents(false);
	//fill with parents
	for (int i = lastOffspringSurvivorIndex + 1; i <= parents.Size(); i++) {
		maxPerformance = 0.0;
		for (int competitor = 1; competitor <= TourSize; competitor++) {
			individual = rs.UniformRandomInteger(eliteCount + 1, parents.Size());
			while (used[individual]) {
				individual = rs.UniformRandomInteger(eliteCount + 1, parents.Size());
			}
			performance = parentsPerf[individual];
			gen = parents[individual];

			//cout << "competidor pa: " << performance << endl;
		...
		TempPopulation[i] = maxIndividual;
		//cout << "Elegido pa: " << maxPerformance<< endl;
		TempPerf[i] = maxPerformance;
		used[MaxindividualIndex] = true;
	}


	//Esta asignación puede dar problemas. Se cambia referencia por algo que vive sólo acá. - Parece no traer problemas
...
void TSearch::printArchUTILITY(TVector<int>& gen) {
	int index = 1;
	for (int neuron = 0; neuron < Ne / 2;neuron++) {
		for (int connection = 0; connection < M;connection ++) {
			cout << neuron << " conectada con " << gen[index++] << endl;
		}
		cout << endl;
	}

	for (int neuron = Ne; neuron < Ne + (Ni / 2);neuron++) {
		for (int connection = 0; connection < M; connection++) {
			cout << neuron << " conectada con " << gen[index++] << endl;
		}
		cout << endl;
...
void TSearch::ReproducePopulationGeneticAlgorithm(void)
{
	...
	switch (ParentSelectMode){
		...
		case TOURNAMENT:{
			...
			for (int i = (ElitePop) + 1 ; i <= psize; i++) {
				...
				TVector<int> used;
				used.SetBounds(1, ParentsTourSize);
				used.FillContents(-1);
				int selectedCount = 0;
				bool inUse;
				for (int competitor = 1; competitor <= ParentsTourSize; competitor++) {
					do { //no repetir en el mismo torneo
						inUse = false;
						individual = rs.UniformRandomInteger(1, psize);

						//checks if are used
						for (int i = 1; i <= ParentsTourSize;i++) {
							if (individual == used[i]) {
								inUse = true;
								break;
							}
						}
	...
	for (int i = 1; i <= psize; i++) {
		...
		printArchUTILITY(Population[i].ints);
	}
	*/


	//mutate elite
	for (int i = 1; i <= ElitePop; i++) {
		MutateVector(Population[i].reals);
		MutateIntVector(Population[i].ints);	
	}

	/*
	cout << "Elite mutada" << endl;
	for (int i = 1; i <= ElitePop; i++) {
		cout << "Individuo "<<i << endl;
		printArchUTILITY(Population[i].ints);
	}
	*/
	...
	/*
	cout << "Después de cruzar/mutar" << endl;
	for (int i = 1; i <= psize; i++) {
		cout << "Individuo "<<i << endl;
		printArchUTILITY(Population[i].ints);
	}
	...
	*/
	...
	SelectNextPopulation(TempPopulation,TempPerf);
}
