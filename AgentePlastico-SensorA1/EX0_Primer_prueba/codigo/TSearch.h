...
enum TArchMutationType { RANDOM_RESET, CREEP_MUTATION };       // Supported architecture mutation modes
enum TParentSelectionMode { SUS, TOURNAMENT };                  // Supported parent selection modes
enum TCrossoverArchWeight { TOGETHER, APART };					

class TSearch {
public:
	// The constructor
	TSearch(int vectorSize = 0, int intVSize = 0, double (*EvalFn)(genotype&, RandomState&) = NULL);
	// The destructor
	~TSearch();
	// Basic Accessors
	...
	int IntsVectorSize(void) { return intsVectorSize; };
	void SetIntsVectorSize(int NewSize);

	int TournamentSize(void) { return TourSize; };
	void SetTournamentSize(int NewSize) { TourSize = NewSize; };

	int ParentsTournamentSize(void) { return ParentsTourSize; };
	void SetParentsTournamentSize(int NewSize) { ParentsTourSize = NewSize; };
	...
	TArchMutationType ArchMutationMode(void) { return AMutMode; };
	void SetArchMutationMode(TArchMutationType newmode) { AMutMode = newmode; };
	TCrossoverArchWeight CrossoverArchWeight(void) { return CrossArchWeig; };
	void SetCrossoverArchWeight(TCrossoverArchWeight newmode) { CrossArchWeig = newmode; };
	...
	void SetArchRandMutationProb(double newProb) { randomMutProb = newProb; };
	...
	void SetEvaluationFunction(double (*EvalFn)(genotype& g, RandomState& rs))
	{
		EvaluationFunction = EvalFn;
	};
	void SetBestActionFunction(void (*BestFn)(int Generation, genotype& g))
	{
	...
	void setCreepMutationStep(int newStep) { creepMutationStep = newStep; };
	// Status Accessors
	int Generation(void) { return Gen; };
	genotype& Individual(int i) { return Population(i); };
	double Fitness(int i) { return fitness(i); };
	double Performance(int i) { return Perf(i); };
	double BestPerformance(void) { return BestPerf; };
	genotype& BestIndividual(void) { return bestGen; };
	// Control
...
private:
	void printArchUTILITY(TVector<int>& gen);
	// Helper Methods
	...
	void RandomizeArchitecture(TVector<int>& Arch);
	void RandomizePopulation(void);
	double EvaluateGen(genotype& Gen, RandomState& rs);
	...
	void SelectNextPopulation(TVector<genotype >& parents, TVector<double>& parentsPerf);
	...
	void MutateIntVector(TVector<int>& Vector);
	void UniformCrossover(genotype& g1, genotype& g2);
	void TwoPointCrossover(genotype& g1, genotype& g2);
	...
	static bool exists(int from, int to, TVector<int> & arch);
	static int getNewConnection(int from);

	// Internal State
	float OffspringPercentageSurvivors = 0.70;
	RandomState rs;
	TVector<RandomState> RandomStates;
	int Gen;
	double GenTime;
	int SearchInitialized;
	TVector<genotype > Population;
	TVector<double> Perf;
	...
	TArchMutationType AMutMode;
	TParentSelectionMode ParentSelectMode;
	TCrossoverArchWeight CrossArchWeig;
	// Search Parameters
	int vectorSize; 		// real vector size
	int intsVectorSize;		// int vector size
	int MaxGens;
	...
	int creepMutationStep;

	// Function Pointers
	double (*EvaluationFunction)(genotype& g, RandomState& rs);
	void (*BestActionFunction)(int Generation, genotype& g);
