...
double NervousSystem::step(double stepSize, double (*distanceInputs)[numDistSensors]) {
...
void NervousSystem::updateRate() {
	...
	for (int mm = 0; mm < M; mm++) { //all conections to motors
		...
		double LWeight = cns.getWeight(leftIndex, mm);
		double RWeight = cns.getWeight(rigthIndex, mm);
