...
int		min_group_time = 20;	// minimal duration of a group (ms)
...
void convertGenotypeToPhenotype(genotype g, Agent& a) {
	...
	double input_bias = MapSearchParameter(g.reals[genotypeSize - 2], -4, 4);
...
double evaluateCateg(Agent& a) {
	//cout << "Un llamado a evaluar" << endl;
	...
	double startingPositions[numTrials * 2];

	//para agente asimetrico
	/*
	for (int i = 0; i < cEvalParams.numTrials; i++) {
		double posOffset = (i) * (100. / (numTrials - 1)) - 50.;
		startingPositions[i] = posOffset;                           // for isCircle trials
		startingPositions[i + cEvalParams.numTrials] = posOffset;   // for !isCircle trials
	}*/
	//cEvalParams.maxDistance = std::abs(startingPositions[0]);
	...
	for (int i = 0; i < cEvalParams.numTrials; i++) {
		double posOffset = (i) * (100. / (cEvalParams.numTrials * 2 - 1)) - 50.;
		startingPositions[i] = posOffset;                           // for isCircle trials
		startingPositions[i + cEvalParams.numTrials] = posOffset;   // for !isCircle trials
	}
	//cEvalParams.maxDistance = std::abs(startingPositions[0]);
	...
	// externalInputs
	double distanceInputs[numDistSensors];
	for (int i = 0; i < numDistSensors; i++) distanceInputs[i]=0.;
	...
	a.changeSTDPStatus(false);

	for (int presentation = 1; presentation <= presentationsNum; presentation++) {
		bool isCircle = 1;
		if (presentation == presentationsNum) a.changeSTDPStatus(false);
		for (int trial = 0; trial < cEvalParams.numTrials * 2; trial++) {
			//cout << "\tTrial-" << trial << endl;
			if (trial == cEvalParams.numTrials) isCircle = !isCircle;
			...
			while (oY > DIAMETER / 2) {
				...
				if (writeFlag == 3 /*&& presentation == presentationsNum*/) {
					cout << t << " " << isCircle << " " << oX << " " << oY << " " << agentPos << " " << offset / integStepSize << " ";
					for (int d = 0; d < numDistSensors; d++) {
		...
		for (i = 0; i < W; i++)
		{
			...
			for (dd = 0; dd < D; dd++)
				for (j = 0; j < delays->getDelayLength(group[i], dd); j++)
				{
					...
					if ((a.getWeight(group[i], delays->getConnNumberByDelay(group[i], dd, j)) > C_rel) & (dd >= D_my_pre[npre[i]]))
					{
						...
						C_postspikes[timing][N_postspikes[timing]] = a.getWeight(group[i], delays->getConnNumberByDelay(group[i], dd, j));	// syn weight
					...
					if (N_fired < polylenmax)
					{
						...
						for (dd = 0; dd < D; dd++)
							for (j = 0; j < delays->getDelayLength(i, dd); j++)
								if ((a.getWeight(i, delays->getConnNumberByDelay(i, dd, j)) > C_rel) | (i >= Ne))
								{
									timing = t + dd + 1;
									J_postspikes[timing][N_postspikes[timing]] = i;				// presynaptic
									D_postspikes[timing][N_postspikes[timing]] = dd;				// delay
			//						L_postspikes[timing][N_postspikes[timing]]=NL+1;			// layer
									C_postspikes[timing][N_postspikes[timing]] = a.getWeight(i, delays->getConnNumberByDelay(i, dd, j));	   // syn weight
...
void	all_polychronous(Agent& a)
{
	FILE* fs;	
	errno_t errorCode = fopen_s(&fs, "..//polyall.dat", "w");
	int	i;
	N_polychronous = 0;

	for (i = 0; i < polylenmax; i++) N_postspikes[i] = 0;

	for (i = 0; i < Ne; i++) polychronous(i,a);
